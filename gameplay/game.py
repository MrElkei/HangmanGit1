...
"""The level of the Game is defined with list of words by difficulty. 
3 levels options: easy, medium or hard words.
...
class Game:
    ...
    def _uzvaras_pazinojums(self):
        ...
        self.display.updateCulumn(0, 
        self.karatavas.getFrame(self.dzivibas))
        ...
        self.display.updateLine(
            2, 1, 
            f"   Dzīvības: {self.dzivibas}")
        self.display.updateLine(
            2, 3, 
            f"   Minēto burtu sk.:{len(self.minetie_burti)}")
        self.display.updateLine(
            2, 4, 
            f"   Minēto vārdu sk.:{len(self.minetie_vardi)}")
    ...
    def _game_over_pazinojums(self):
        time_elapsed = datetime.now() - self.start_time
        self.display.updateCulumn(
            0, self.karatavas.getFrame(self.dzivibas))
        ...
        self.display.updateLine(
            2, 3, f"   Minēto burtu sk.: {len(self.minetie_burti)}")
        self.display.updateLine(
            2, 4, f"   Minēto vārdu sk.: {len(self.minetie_vardi)}")
    ...
    def play(self):
        ...
        self.display.updateLine(
            1, 0, 
            f"   --== Laipni lūgti karātāvās! ==--")
        self.display.updateLine(
            1, 2, 
            f"Uzmini vārdu, kas sastāv no {len(self.vards)} burtiem.")
        ...
        self.display.updateLine(
            2, 2, 
            f"   Vārda  garums: {len(self.vards)}")
        self.display.updateLine(
            2, 6, 
            f"Grūtības pakāpe: {self.grutibas_pakape}.")
        self.display.updateLine(
            2, 7, 
            f"Raksti EXIT, lai izietu.")
        ...
        while not self.vards_atminets:
            ...
            self.display.updateLine(
                1, 6, 
                f"Tavs progress: {self.aizklats_vards}")
            self.display.updateLine(
                2, 1, f"   Dzīvības: {self.dzivibas}")
            self.display.updateLine(2, 3, 
                f"   Minēto burtu sk.: {len(self.minetie_burti)}")
            self.display.updateLine(
                2, 4, 
                f"   Minēto vārdu sk.: {len(self.minetie_vardi)}")
            ...
            while not deriga_ievade:
                try:
                    ...
                    if minejums.isalpha():
                        ...
                        # Nederīgs minējums
                        self.display.updateLine(
                            1, 2, "Netika ievadīts burts vai vārds. Lūdzu mēģini vēlreiz.")
                        self.display.updateLine(
                            1, 3, " ")
                        self.display.updateLine(
                            1, 4, " ")
            ...
            if len(minejums) == 1:
                ...
                if minejums in self.minetie_burti:
                    ...
                    self.display.updateLine(
                        1, 2, "Šo burtu tu jau esi minējis!")
                    self.display.updateLine(
                        1, 3, "Esi uzmanīgāks!")
                else:
                    ...
                    if minejums in self.vards:
                        # Šis burts ir vārdā. 
                        # Pievienojam to uzminēto burtu sarakstam.
                        ...
                        self.dzivibas -= 1
                        self.display.updateLine(
                            1, 2, "Diemžēl tu neuzminēji, mēģini vēlreiz!")
                        self.display.updateLine(
                            1, 3, f'Minamajā vārdā nav burta "{minejums}"')
            else:
                # Minējums ir vārds
                ...
                if minejums in self.minetie_vardi:
                    ...
                    self.display.updateLine(
                        1, 2, "Šo vārdu tu jau esi minējis!")
                    self.display.updateLine(
                        1, 3, "Esi uzmanīgāks!")
                elif minejums == "EXIT":
                    self._game_exit()
                    ...
                    if minejums == self.vards:
                        ...
                        if len(self.vards) == len(minejums):
                            ...
                            self.display.updateLine(
                                1, 2, "Diemžēl tu neuzminēji!")
                        elif len(self.vards) > len(minejums):
                            # Minētais vārds ir par īsu
                            self.display.updateLine(
                                1, 2, "Ievadītais vārds ir par īsu!")
                        else:
                            # Minētais vārds ir par garu
                            self.display.updateLine(
                                1, 2, "Ievadītais vārds ir par garu!")
            ...
            # Atlaujam atvert kadu no neuzminetajiem burtiem, 
            # ja ir palikusi 1 dziviba
            # Izmantojot papildiespeju tiek izmantots nejausi izvelets burts, 
            # kas nav atminets
            if self.dzivibas == 1 and self.papildiespeja == False:
                self.display.updateLine(
                    1, 4, "Ak vai tev palika tikai viena dzīvība.")
                self.display.updateLine(
                    1, 5, "Lai tev palīdzētu, sniegsim tev papildiespēju!")
                self.display.refresh()
                ...
                try:
                    ...
                    if atbilde in ["JĀ", "JA", "J", "Yes", "Y"]:
                        ...
                        self.display.updateLine(
                            1, 2, 
                            f'Nezināmais vārds satur burtu "{atklatais_burts}".')
                        self.display.updateLine(
                            1, 3, " ")
                        self.display.updateLine(
                            1, 4, " ")
                        ...
                        self.display.updateLine(
                            1, 2, f'Tev ir palikusi pēdējā dzīvība. Saņemies!')
                        self.display.updateLine(
                            1, 3, " ")
                        self.display.updateLine(
                            1, 4, " ")
