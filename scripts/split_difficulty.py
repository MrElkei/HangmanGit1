...
"""Adding words.txt file, which contains
all the guessable words, location"""
...
# print(words_list)
"""1st difficulty setting calculates word length - the longer the word,
...
"""2nd difficulty setting calculates the amount of most popular letters
for guessing in the word. The higher the amount of these letters,
the easier the difficulty"""
...
for n in range(0, len(words_list)):
    ...
    """Score count for word length - for more letters in the word higher
    score is added, which gives additional points to word difficulty"""
    word_len_score = 0
    if word_len[n] < 7:
        pass
    elif word_len[n] < 10:
    ...
    unpopular_letter_repetitiveness = (int((unpopular_letters_count[n] * 100) 
    / word_len[n]))
    ...
    popular_letter_repetitiveness = (int((popular_letters_count[n] * 100) 
    / word_len[n]))
    ...
    score = (word_len_score + letter_repetitiveness_score 
    + popular_letter_score + unpopular_letter_score)
...
"""Function for word list split in 3 difficulties - easy, medium
and hard. Also added a chance for user
...
"""3 different files for difficulties added, which include only words 
of the needed difficulties in the files with acording name"""
